...
test.describe("CaskrApp", async() => {
    test.beforeEach("Inicios de sesión", async ({ page }) => {
        ...
        await page.getByTestId('inputCorreo').fill('majo67@prueba.com'); 
    ...
    test("Agregar", async ({ page }) => {
        ...
        var nota = await page.locator("No puedes añadir un equipo porque tienes partidos agendados")
        var boton = await page.locator('//span[text()="Agregar equipo"]').isEnabled()
        console.log(boton)
