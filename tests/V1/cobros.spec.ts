...
test.describe("Pestaña_P", async () => {
  test("Collections", async ({ page }) => {
    ...
    await page
      .getByRole("textbox", { name: "¿A quién va dirigido el cobro?" })
      .click();
    var opciones = page.locator(
      '[data-combobox-option="true"][role="option"]:visible'
    );
    ...
    fin = Date.now();
    console.log(
      "Tiempo de carga para añadir productos: " + (fin - inicio) + "ms"
    );
  });
  ...
  test("Productos", async ({ page }) => {
    ...
    await page
      .getByPlaceholder("Ej. Arbitraje", { exact: true })
      .fill(fakerES_MX.word.words());
    await page
      .getByPlaceholder("Ej. Arbitraje de un partido")
      .fill(fakerES_MX.lorem.lines(1));
    await page
      .getByPlaceholder("$600.00 + IVA + Comisión")
      .fill(faker.number.bigInt().toString());
    await page.getByRole("textbox", { name: "Recurrencia" }).click();
    var opciones = page.locator(
      '[data-combobox-option="true"][role="option"]:visible'
    );
    await Random(opciones);
    await page
      .getByRole("textbox", { name: "Fecha de inicio de cobro" })
      .click();
    var fecha1 = new Date();
    var mes = fecha1.toLocaleString("en-EN", { month: "long" });
    var fecha = [Math.floor(Math.random() * 28) + 1].toString() + " " + mes;
