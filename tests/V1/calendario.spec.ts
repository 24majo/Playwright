...
test("Generar", async ({ page }) => {
  ...
  if (visible) {
    ...
    if (disponible == null) {
      ...
      inicio = Date.now();
      await page
        .getByRole("tab", { name: "Por jornadas" })
        .waitFor({ state: "visible" });
      fin = Date.now();
      console.log(
        "Tiempo de generación de calendario: " + (fin - inicio) + "ms"
      );
    }
  }
});
...
test("Jornada", async ({ page }) => {
  ...
  var j_num = await page
    .getByRole("tab", { name: "J - ", exact: false })
    .count();
  ...
  for (var i = 1; i <= j_num; i++) {
    ...
    for (var j = 1; j <= count; j++) {
      ...
      if (j != count) {
        await page
          .getByRole("tab", { name: new RegExp(`J - ${i}`) })
          .first()
          .click({ force: true });
...
test("Flex_Eliminacion", async ({ page }) => {
  ...
  await page
    .getByRole("textbox", { name: "¿Cúantos equipos quieres" })
    .fill("4");
  ...
  var programar = page
    .locator("div")
    .filter({ hasText: `.+Programar partido.+` })
    .getByRole("button");
...
test("Todos", async ({ page }) => {
  ...
  await page.pause();
  await page
    .getByRole("tab", { name: "Todos los partidos" })
    .click({ force: true });
  await page
  ...
  if (visible) {
    ...
    if (activa) {
      // Ir a la siguiente página disponible
      console.log("Caso 2");
      await flecha.click();
