...
test.describe("CaskrApp", async () => {
  ...
  test("Agregar", async ({ page }) => {
    ...
    for (var i = 0; i < 10; i++) {
      ...
      await page
        .getByLabel(/Añadir/)
        .getByRole("button", { name: "Agregar cancha" })
        .click({ force: true });
      inicio = Date.now();
      await page
        .locator('[aria-modal="true"][role="dialog"]:visible')
  ...
  test("Eliminar", async ({ page }) => {
    ...
    for (var i = 0; i < 4; i++) {
      ...
      await page
        ...
        .click({ force: true });
      var error = await page
        .locator("text=Está cancha no puede ser")
        .isVisible();

      if (error) console.log("Caso 2");
      else {
        console.log("Caso 1");
        inicio = Date.now();
        await page
        ...
        fin = Date.now();
        console.log(
          "Tiempo de eliminación de cancha: " + (fin - inicio) + "ms"
        );
      }
...
export const agregar_cancha = async (page: any) => {
  ...
  await page
    ...
    .fill("Cancha " + fakerES_MX.location.city());
