...
test("Agregar", async ({ page }) => {
  ...
  for (var i = 0; i < 10; i++) {
    ...
    inicio = Date.now();
    await page
      .locator('[data-modal-content="true"][role="dialog"]:visible')
...
test("Editar", async ({ page }) => {
  for (var i = 0; i < 5; i++) {
    ...
    await page
      .locator('//input[@name="telefono"]')
      .fill(faker.number.int({ min: 1000000000, max: 9999999999 }).toString());
...
export const agregar_arbitro = async (page: any) => {
  await page
    .getByRole("button", { name: "Agregar árbitro" })
    .click({ force: true });
  await page
    .locator('[data-modal-content="true"][role="dialog"]:visible')
    .waitFor();
  ...
  await page
    .locator('//input[@name="telefono"]')
    .fill(faker.number.int({ min: 3000000000, max: 9999999999 }).toString());
  await page
    .locator('//input[@name="email"]')
    .fill(fakerES_MX.internet.email());
};
