...
test("Active Delete", async ({ page }) => {
  ...
  await page
    .locator('[aria-modal="true"][role="dialog"]:visible')
    .waitFor({ state: "visible" });
  var modal1 = await page
    .getByRole("heading", { name: "¿Estás seguro de eliminar el" })
    .isVisible();
  console.log("Modal confirmar eliminar: " + modal1);
  var modal2 = await page
    .getByRole("heading", { name: "No puedes eliminar este torneo" })
    .isVisible();
...
test("Inactivo Eli", async ({ page }) => {
  await page
    .getByRole("tab", { name: "Torneos inactivos" })
    .click({ force: true });
  await page
    .getByLabel("Torneos inactivos")
    .locator("button")
    .nth(2)
    .click({ force: true });
...
test("Formal", async ({ page }) => {
  await page.pause();
  var mod = await page
    .getByRole("button", { name: "Se parece a este" })
    .first();
