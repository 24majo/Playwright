...
test.beforeEach(async ({ page }) => {
  ...
  await page
    .getByRole("tab", { name: "Torneos activos" })
...
test("1. Liga", async ({ page }) => {
  await FunctionsLiga(
    page,
    await page.getByRole("button", { name: "Se parece a este" }).first(),
    "Liga",
    0,
    "Formal"
  );
...
test("6. Liga Flexible", async ({ page }) => {
  await FunctionsLiga(
    page,
    await page.getByRole("button", { name: "Se parece a este" }).nth(1),
    "Liga",
    0,
    "Flexible"
  );
...
async function FunctionsElimination(
  page: any,
  modalidad: any,
  formato: string,
  grupo: number,
  ...
  if (formato == "Eliminación directa")
    await ResultsEliminacion(
      page,
      mod,
      formato
    ); // Registrar resultados de la eliminacion
  else await ResultsEliminacionVuelta(page, mod, formato);
}
...
async function Create_Fixture(
  ...
  if (
    modalidad == "Flexible" &&
    (formato == "Liga (ida y vuelta)" || formato == "Liga")
  ) {
    ...
    await page
      .locator('[data-truncate="end"]')
      .first()
      .waitFor({ state: "visible" });
    await page.getByRole("button", { name: "Empezar a programar" }).click();
  } else {
    await page.getByRole("link", { name: "Calendario" }).click();
    var calendario = await page.getByRole("button", {
      name: "Crear el calendario",
    });
...
async function Enfrentamientos(page: any, modalidad: any) {
  ...
  await page
    .locator('[placeholder="Busqueda general"]')
    .waitFor({ state: "visible" });
  var boton = page
    .getByRole("row", { name: new RegExp(`.+ NO PROGRAMADO .+`) })
    .getByRole("button")
    .first();
  await page.pause();
  ...
  while (!visible) {
    ...
    if (modalidad == "Flexible") {
      await page
        .getByRole("textbox", { name: "Selecciona equipo local" })
        .click();
      var local = page.locator(
      ...
      await page
        .getByRole("textbox", { name: "Selecciona equipo visitante" })
        .click();
      var visitante = page.locator(
        '[data-combobox-option="true"][role="option"]:visible'
      );
  ...
  if (visible) {
    ...
    if (activa == "false") {
      ...
      while (!visible) {
        // Hay partidos por programar en la siguiente pestaña
        boton.click({ force: true });
        await boton.waitFor({ state: "visible" });
        await page
          .locator('[aria-modal="true"][role="dialog"]:visible')
...
async function ResultsLiga(page: any, modalidad: string, formato: string) {
  await page.getByRole("link", { name: "Resultados" }).click();
  await page
    .locator("text=Registro de resultados")
    .waitFor({ state: "visible" });
  var registrar = page
    .getByRole("row", { name: new RegExp(`.+ Registrar`) })
    .getByRole("button");
...
async function ResultsEliminacion(
  page: any,
  modalidad: string,
  formato: string
) {
...
async function ResultsEliminacionVuelta(
  page: any,
  modalidad: any,
  formato: string
) {
  ...
  var ida = await page.getByRole("button", { name: "Registrar Ida" });
  var vuelta = await page.getByRole("button", { name: "Registrar vuelta" });
  var act_ida = await page
    .getByRole("button", { name: "Actualizar ida" })
    .last();
  var act_vuelta = await page
    .getByRole("button", { name: "Actualizar vuelta" })
    .last();
...
async function Results(
  ...
  await page
    .locator('input[name="puntos_visitante"]')
    .fill(visitante.toString());
  await page.pause();
  ...
  await page.waitForTimeout(1000);
  var ganador = await page
    .getByRole("dialog", { name: "Confirmar cambio de resultado" })
    .getByRole("banner");
  ...
  await page.waitForTimeout(500);
  var share = await page
    .locator("div")
    .filter({ hasText: /^¡JORNADA TERMINADA!¿Lo compartimos\?$/ })
    .first();
...
async function Ganador(page: any) {
  await page
    .getByRole("link", { name: "Tabla de posiciones" })
    .click({ force: true });
  var ganador = await page
    .locator('[role="gridcell"][col-id="equipos"][aria-colindex="3"]')
    .first();
...
async function GanadorInicio(page: any, formato: string, equipo: string) {
  ...
  if (visible_ganador) {
    ...
    console.log(
      "El ganador de la tabla de posiciones no coincide con el ganador del inicio"
    );
  }
}
