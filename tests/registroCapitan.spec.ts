...
test("Unirme", async ({ page }) => {
    ...
    await page.goto('https://caskr.app/registroCapitan/1747683472277/1747683472079')
    ...
    inicio = Date.now()
    await page.getByRole('button', { name: 'Editar mi card' }).waitFor({state: 'visible'})
    fin = Date.now()
...
async function Sesion(page: any) {
    ...
    await page.getByRole('textbox', { name: 'Teléfono' }).fill('4731201251')
